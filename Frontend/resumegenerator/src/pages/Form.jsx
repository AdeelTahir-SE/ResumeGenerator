...
const handleFileChange = (event, type) => {
  ...
  const [formData, setFormData] = useState({
    profileImage: "",
    id: user.id, // Assuming the id will be set later
    company: {
    ...
    aboutMeData: {
      para1: "Default paragraph 1",
      para2: "Default paragraph 2",
    ...
    education: [{ details: "Default education details" }],
    interests: [{ name: "Default Interest" }],
    projects: [
      {
      ...
      if (typeof formData[key] === "object" && !Array.isArray(formData[key])) {
        ...
        formData[key].forEach((item, index) => {
          Object.keys(item).forEach((subKey) => {
            if (!item[subKey]) {
              newErrors[`${key}-${index}-${subKey}`] = `${subKey
                .replace(/([A-Z])/g, " $1")
                .toUpperCase()} is required for item ${index + 1}.`;
            }
          });
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/resumedata`, {
        method: "POST",
        headers: {
        ...
        await fetch(`http://localhost:3000/api/templates/images`)
       

      if (!response.ok) {
        const errorData = await response.json();
        alert(
          `Error: ${errorData.error || "An error occurred. Please try again."}`
        );
      } else {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-gray-700 font-semibold mb-1">
              Input the Profile image
            </label>
            <input
              className="border border-gray-300 rounded-lg p-2 w-full focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105"
              type="file"
              accept="image/png"
          ...
          {/* Company Information */}
          {["name", "jobtitle"].map((field) => (
            <div key={field} className="form-group mb-4">
              <label className="block text-gray-700 font-semibold mb-1">
                Company {field.replace(/([A-Z])/g, " $1").toUpperCase()}:
              </label>
              <input
                type="text"
                name={`company-${field}`}
                ...
                className={`border border-gray-300 rounded-lg p-2 w-full focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105 ${
                  errors[`company-${field}`] ? "border-red-500" : ""
                }`}
              />
              {errors[`company-${field}`] && (
          ...
          {["para1", "para2"].map((field) => (
            <div key={field} className="form-group mb-4">
              <label className="block text-gray-700 font-semibold mb-1">
                About Me {field.replace(/([A-Z])/g, " $1").toUpperCase()}:
              </label>
              <textarea
                name={`aboutMeData-${field}`}
                value={formData.aboutMeData[field]}
                onChange={handleChange}
                className={`border border-gray-300 rounded-lg p-2 w-full h-24 resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105 ${
                  errors[`aboutMeData-${field}`] ? "border-red-500" : ""
                }`}
              />
              {errors[`aboutMeData-${field}`] && (
                <p className="text-red-500 text-sm">
                  {errors[`aboutMeData-${field}`]}
                </p>
              )}
          ...
          <div className="form-group mb-4">
            ...
            <textarea
              ...
              onChange={handleChange}
              className={`border border-gray-300 rounded-lg p-2 w-full h-24 resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105 ${
                errors["whyShouldHireYou-reason"] ? "border-red-500" : ""
              }`}
            />
            {errors["whyShouldHireYou-reason"] && (
              <p className="text-red-500 text-sm">
                {errors["whyShouldHireYou-reason"]}
              </p>
            )}
            ...
            {formData.skills.map((skill, index) => (
              <div key={index} className="mb-2 flex space-x-2">
                <input
                  ...
                  className={`border border-gray-300 rounded-lg p-2 flex-1 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105 ${
            ...
            {formData.education.map((edu, index) => (
              <div key={index} className="mb-2 flex space-x-2">
                <input
                  ...
                  className={`border border-gray-300 rounded-lg p-2 flex-1 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105 ${
                    errors[`education-${index}-details`] ? "border-red-500" : ""
                  }`}
                />
                {index > 0 && (
                  <button
          ...
          <button
            ...
            onClick={() =>
              setFormData({
                ...formData,
                education: [...formData.education, { details: "" }],
              })
            }
            className="bg-purple-800 hover:bg-purple-500 text-white rounded-lg px-4 py-2"
          >
            ...
            {formData.interests.map((interest, index) => (
              <div key={index} className="mb-2 flex space-x-2">
                <input
                  ...
                  onChange={(e) => handleChange(e, index, "name")}
                  className={`border border-gray-300 rounded-lg p-2 flex-1 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105 ${
                    errors[`interests-${index}-name`] ? "border-red-500" : ""
                  }`}
                />
                {index > 0 && (
                  <button
            ...
            {formData.projects.map((project, index) => (
              <div key={index} className="mb-4">
                ...
                <textarea
                  ...
                  className={`border border-gray-300 rounded-lg p-2 w-full h-24 resize-none mt-2 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105 ${
                    errors[`projects-${index}-content`] ? "border-red-500" : ""
                  }`}
                />
                <input
                  type="text"
                  ...
                  className={`border border-gray-300 rounded-lg p-2 w-full mt-2 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105 ${
                    errors[`projects-${index}-tags`] ? "border-red-500" : ""
                  }`}
                />
                {index > 0 && (
                  <button
          ...
          <button
            type="submit"
            className="bg-purple-800 text-white rounded-lg px-4 py-2 w-full mt-4 hover:bg-purple-500 transition-transform duration-500 ease-in-out transform hover:scale-105"
          >
            Submit
          </button>
        </form>
